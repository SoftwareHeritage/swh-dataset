...
from ..relational import MAIN_TABLES, TABLES
...
ARROW_TYPE_MAP = {
    ...
    "timestamp": pyarrow.timestamp("s", tz="UTC"),  # FIXME: loss of precision
...
class BaseWriter:
    def __init__(self, schema: pyarrow.Schema):
        self.schema = schema
        self.buffers: list[list[Any]] = [[] for _ in schema.names]
        self.current_row = 0
    ...
    def write_batch(self, batch: pyarrow.RecordBatch) -> None:
        raise NotImplementedError(f"{self.__class__.__name__}.write_batch()")
    ...
    def __del__(self):
        if self.buf[0]:
            raise ValueError("BaseWriter deleted without flushing")


Writer = TypeVar("Writer", bound=BaseWriter)


class BaseArrowExporter(ExporterDispatch, Generic[Writer]):
    """
    Base implementation of an exporter which writes the entire graph dataset as
    ORC or Parquet files. Useful for large scale processing, notably on cloud instances
    (e.g BigQuery, Amazon Athena, Azure).
    """

    remove_pull_requests: bool
    with_data: bool
    max_rows: dict[str, int]

    def _init_arrow_exporter(self):
        invalid_tables = [
    ...
    def _reset(self) -> None:
        ...
        # {table: UUID}
        self.uuids: dict[str, uuid.UUID] = {}
        # {table: UUID}
        self.uuid_main_table: dict[uuid.UUID, str] = {}
    ...
    def get_writer_for(self, table_name: str, unique_id=None) -> Writer:
        ...
        if table_name not in self.buffers:
            ...
            self.writers[table_name] = self.new_writer_for(table_name, unique_id)
            self.uuids[table_name] = unique_id

        return self.writers[table_name]

    def new_writer_for(self, table_name: str, unique_id: uuid.UUID) -> Writer:
        raise NotImplementedError(f"{self.__class__.__name__}.new_writer_for()")
